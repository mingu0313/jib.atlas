...
// 의 RoomStudioScene3D 동적 로드와 같은 이유·같은 패턴. 로딩 문구만 다른
// 래퍼 두 개(StudioPreviewPanel.tsx와 같은 이유 — dynamic()의 loading은
// 렌더 시점 props를 못 받는다) — 실제 청크는 동일해서 중복 로드는 없다.
const StudioRoomSceneKo = dynamic(
...
const StudioRoomSceneEn = dynamic(
  () => import("@/components/atlas/StudioRoomScene").then((m) => m.StudioRoomScene),
  { ssr: false, loading: () => <div className="flex h-[420px] items-center justify-center text-sm text-muted">Loading 3D view…</div> },
);
 ...
 */
export function StudioRoomViewer({ room, lang = "ko" }: { room: StudioRoomSnapshot; lang?: "ko" | "en" }) {
  const isEn = lang === "en";
  const StudioRoomScene = isEn ? StudioRoomSceneEn : StudioRoomSceneKo;
  return (
    <div className="relative h-[420px] w-full overflow-hidden rounded-[16px] bg-panel">
      ...
      <span className="label-mono pointer-events-none absolute bottom-4 left-4 rounded-full bg-card/85 px-3 py-1.5 text-[9px] text-faint">
        {isEn ? "Drag to rotate · Scroll to zoom" : "드래그해서 돌려보세요 · 스크롤로 확대"}
      </span>
    </div>
  );
}
